...
export const UpdateNoteForm = ({noteId, noteName,noteDescription, noteImportant, noteStatus,noteDueDate, visibleUpdateForm}) => {
        ...
        if(noteExists){
            ...
            if(response){
                ...
                notesServices.updateNote(id, updatedNote).then((updatedNote)=>{
                    handleUpdateNote(notes.map((note)=> note.id !==id? note: updatedNote));
    ...
    return (
        <form onSubmit={updateNoteEvent} className="article__form">
            ...
            <input type="text" id="article__name" name="title" value={updateName} onChange={handleNameValueUpdate} />

            <label htmlFor="article__description">Description:</label>
            <input type="text" id="article__description" name="description"  value={updateDescription} onChange={handleDescriptionValueUpdate}/>

            <label htmlFor="article__important">Important:</label>
            <input type="text" id="article__important" name="important" value={updateImportant} onChange={handleImportantValueUpdate} />

            <label htmlFor="article__status">Status:</label>
            <select id="article__status" name="status" value={updateStatus} onChange={handleStatusValueUpdate}>
            ...
            <input type="date" id="article__duedate" value={updateDueDate} onChange={handleDueDateValueUpdate} />
